...
def score_from_meta_cache(
    ...
    **_unused,
    ...
    """Return 0-100 similarity for one roast ID using preloaded metadata cache."""
    ...
    roast_entries = list(meta_cache.items())
    ...
    available_process_labels = {
        str(m.get("process", "")).strip().lower()
        for _, m in roast_entries
        if str(m.get("process", "")).strip()
    }
    available_process_families = {
        family
        for _, m in roast_entries
        for family in [_normalize_process_family(str(m.get("process", "")))]
        if family is not None
    }
    available_modifier_base_pairs = {
        _process_profile(str(m.get("process", "")))
        for _, m in roast_entries
    }
    ...
    theoretical_max = 40 + 25 + 20 + 10 + 5 + 8 + 4 + (420 if current_is_gesha else 0)
    return round(max(0.0, min(100.0, score / theoretical_max * 100)), 1)
